...
class TestDeleteOption(unittest.TestCase):
    biz_date = None
    PATH = "/trade/entry/delete/option/trade"
    HOST = get_yaml(Test_config_path)['HOST']
    URL = HOST + PATH
    BODY = {
        "fourEyeCheckStatus": 2,
        "lastUpdateBy": "admin",
        "tradeEntryIds": []
    }
    ...
    def test_deleteOption_001(self):
        """删除一条新增的option"""
        body = copy.deepcopy(self.BODY)
        body['tradeEntryIds'].append(self.Trade_id)
        res = Request.post_request(url=self.URL, body=body)
        self.assertEqual(200, res.status_code)
        self.assertEqual("Success", res.json()["msg"])

    def test_deleteOption_002(self):
        """不传入trade_id，查看接口的响应"""
        res = Request.post_request(url=self.URL, body=self.BODY)
        self.assertEqual(422, res.status_code)
        self.assertEqual("Cannot Be Empty", res.json()["data"][0]["msg"])

    def test_deleteOption_003(self):
        """入参为空字典，查看接口响应"""
        body = {}
        res = Request.post_request(url=self.URL, body=body)
        self.assertEqual(500, res.status_code)
        self.assertEqual("action failed", res.json()["msg"])

    def test_deleteOption_004(self):
        """入参为空，查看接口响应"""
        body = ''
        res = Request.post_request(url=self.URL, body=body)
        self.assertEqual(500, res.status_code)
        self.assertIn("JSON parse error", res.json()["data"])

    def test_deleteOption_005(self):
        """传入不存在的Trade_id，查看接口响应"""
        body = copy.deepcopy(self.BODY)
        body['tradeEntryIds'].append("random001")
        res = Request.post_request(url=self.URL, body=body)
        self.assertEqual(200, res.status_code)
